...
| `parquet/virtual_provenance.parquet` | 8,193 rows | generated by the C++ harness with 2,048-row groups; checks original row positions after selecting noncontiguous groups and filtering rows. |
| `parquet/virtual_physical_names.parquet` | 2 rows | generated by the C++ harness; physical columns named `filename`, `file_index`, and `file_row_number`, including a NULL filename, verify binding precedence. |
...
The virtual-column fixtures also include `virtual_nested_only.parquet`. Strict S3 tests cover metadata-only nested virtual projections through both SQL entries, prune a nonexistent source before opening it, and assert no data GET/payload/H2D activity after footer work. Footer probe overread is outside that data-stage assertion. Footer cache consistency/version validation belongs to a separate PR (TODO: add link); scan tests reuse the existing cache contract.
